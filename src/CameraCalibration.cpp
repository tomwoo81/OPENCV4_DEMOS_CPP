...
" \n"
" example command line for calibration from a number of stored images:\n"
"   calibration -w=4 -h=5 -s=0.025 -o=camera.yml -op -oe ccImages\n"
" where ./ccImages is the directory storing the image files\n";
...
static void help(char** argv)
{
    printf( "This is a camera calibration sample.\n"
        ...
        "                              #  - directory storing a number of image files of the board\n"
...
static bool createFileList( const string& path, vector<string>& l )
{
	if (path.empty())
		return false;
    ...
    vector<string> filenames = orz::FindFilesRecursively(path, -1);
    for (auto& filename: filenames) {
    	l.emplace_back(path + "/" + filename);
    }
...
int main( int argc, char** argv )
{
    ...
    if ( isdigit(parser.get<string>("@input_data")[0]) )
    	...
    	inputData = parser.get<string>("@input_data");
    ...
    if( !inputData.empty() )
    {
        if( !videofile && createFileList(inputData, imageList) )
            mode = CAPTURING;
        else
            capture.open(samples::findFileOrKeep(inputData));
    }
    else
