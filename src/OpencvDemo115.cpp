...
int OpencvDemo115() {
    ...
    // 加载KNN分类器进行预测
	cv::Ptr<cv::ml::KNearest> pKNN = cv::ml::KNearest::load("models/knn_digits.yml");
	cv::Mat results;
	CV_LOG_INFO(CV_LOGTAG_GLOBAL, "KNN prediction starts...");
	pKNN->findNearest(testData, 5, results);
	CV_LOG_INFO(CV_LOGTAG_GLOBAL, "KNN prediction done.");
	int correct_count = 0;
	...
	CV_LOG_INFO(CV_LOGTAG_GLOBAL, 
				cv::format("KNN prediction - image 1: label: %d, prediction: %d", testLabels.at<int>(0, 0), (int)results.at<float>(0, 0)));
	cv::imshow("KNN prediction - image 1", t1);

	CV_LOG_INFO(CV_LOGTAG_GLOBAL, 
				cv::format("KNN prediction - image 2: label: %d, prediction: %d", testLabels.at<int>(1, 0), (int)results.at<float>(1, 0)));
