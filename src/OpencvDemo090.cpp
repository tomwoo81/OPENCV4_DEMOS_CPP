...
int OpencvDemo090() {
    ...
    std::vector<cv::Point> tracking_path;

    cv::Mat dst, result;
	std::string winName = "object tracking and trace rendering by continously adaptive mean shift method";
	...
	while (capture.read(src)) {
		...
		if ((tracking_box.center.x > 0) && (tracking_box.center.y > 0)) {
            tracking_path.emplace_back(tracking_box.center);
        }
        ...
        // 绘制对象移动轨迹
        std::vector<std::vector<cv::Point>> tracking_paths = {tracking_path};
        cv::polylines(dst, tracking_paths, false, cv::Scalar(255, 0, 0), 2);
        size_t num_pts = tracking_path.size();
        if (num_pts >= 2) {
            cv::arrowedLine(dst, tracking_path[num_pts - 2], tracking_path[num_pts - 1], cv::Scalar(255, 0, 0), 2, cv::LINE_8, 0, 0.5);
        }
