...
static void process_frame(const cv::Mat& src, cv::Mat& dst) {
	...
	cv::TermCriteria criteria = cv::TermCriteria(cv::TermCriteria::COUNT + cv::TermCriteria::EPS, 40, 0.001);
	...
	for (size_t i = 0; i < corners.size(); i++) {
        CV_LOG_INFO(CV_LOGTAG_GLOBAL, cv::format("refined corner location [%zu]: (%.3f, %.3f)", 
