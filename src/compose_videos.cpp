...
int main(int argc, const char *argv[])
{
    ...
    // Generate a composite video from foreground and background videos

	cv::VideoCapture capture_foreground("videos/01.mp4");
	if (!capture_foreground.isOpened()) {
		CV_LOG_ERROR(CV_LOGTAG_GLOBAL, "could not read a video file!");
		return cv::Error::StsError;
	}

	cv::VideoCapture capture_background("videos/dushuhu.mp4");
	if (!capture_background.isOpened()) {
		CV_LOG_ERROR(CV_LOGTAG_GLOBAL, "could not read a video file!");
		return cv::Error::StsError;
	}
    ...
    if (!capture_foreground.read(foreground)) {
        CV_LOG_ERROR(CV_LOGTAG_GLOBAL, "could not read a frame!");
        return cv::Error::StsError;
    }

    if (!capture_background.read(background)) {
        CV_LOG_ERROR(CV_LOGTAG_GLOBAL, "could not read a frame!");
        return cv::Error::StsError;
    }

    cv::Mat result;
	std::string windowTitle = "video composition";
	cv::namedWindow(windowTitle, cv::WINDOW_NORMAL);

    while (capture_foreground.read(foreground)) {
        // BGR space -> HSV space
        cv::Mat hsv;
		...
		if (result.empty()) {
			result = cv::Mat::zeros(cv::Size(w * 2, h * 2), foreground.type());
		}
