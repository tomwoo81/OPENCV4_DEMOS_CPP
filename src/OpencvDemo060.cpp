...
int OpencvDemo060() {
	...
	cv::Mat src = cv::imread("images/morph01.png");
	if (src.empty()) {
	...
	cv::HoughLinesP(binary, lines, 1, CV_PI / 180, 80, 30, 10);
	...
	for (size_t i = 0; i < lines.size(); i++) {
		...
		cv::line(dst, pt1, pt2, cv::Scalar(0, 255, 0), 1, cv::LINE_AA);
	}
	...
	cv::putText(result, "image with results of linear detection", cv::Point(w + 10, 30), cv::FONT_ITALIC, 0.6, cv::Scalar(0, 0, 255), 1);
